...
def CreateTheShellFile(argv):
    ...
    shell_file.write('export VO_CMS_SW_DIR='+os.getenv("VO_CMS_SW_DIR","/afs/cern.ch/cms")+'\n')
    ...
    if   argv[0]=='BASH':
        ...
        shell_file.write('cmsRun ' + Path_Cfg + '\n')
    else:
    ...
    #shell_file.write('mv '+ Jobs_Name+'* '+Farm_Directories[3]+'\n')
    shell_file.write('mv *'+ Jobs_Name +"_"+ Jobs_Int+'* '+Farm_Directories[3]+'\n')
    shell_file.close()
...
def AddJobToCmdFile():
    ...
    if useLSF:
        cmd_file.write("bsub -q " + Jobs_Queue + " -J " + Jobs_Name+Jobs_Index + " -o " + Path_Log + ".out -e " + Path_Log + ".err '" + Path_Shell + " 0 ele'\n")
    else:
        cmd_file.write('\n')
...
def SendCluster_Create(FarmDirectory, JobName):
    ...
    if(command_out.find("command not found")<0):
        useLSF = True
    else:
        useLSF = False;
